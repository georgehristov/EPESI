...
class Utils_AttachmentCommon extends ModuleCommon {
	...
	public static function get_last($group=null,$group_starts_with=false) {
        $attachments = Utils_RecordBrowserCommon::get_records('utils_attachment',array('id'=>self::get_where($group,$group_starts_with)), [], [':created_by' => 'DESC']);
        $ret = [];
        foreach($attachments as $a) {
        	$ret = $a;
        	$ret['permission_owner'] = $ret['permission_by'] = $ret['note_by'] = $ret['created_by'];
        	$ret['local'] = '';//deprecated here
        	$ret['note_revision'] = 0; //deprecated
        	$ret['note_on'] = $ret['created_on'];
        	$ret['text'] = $ret['note'];
        	break;
        }
        return $ret;
